...
const addProperty = async (req,res) => {
  const form = new formidable.IncomingForm({ multiples: true });
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        const errors = [];
        const locationData = location.length > 0 ? JSON.parse(location[0]) : undefined;

        if (!locationData || !locationData.coordinates || locationData.coordinates.length !== 2) {
            errors.push("Invalid location data");
        }

        if (!title[0] || title[0].trim().length === 0) errors.push("Title is required");
        if (!description[0] || description[0].trim().length === 0) errors.push("Description is required");
        if (isNaN(price[0]) || parseFloat(price[0]) <= 0) errors.push("Price must be a positive number");
